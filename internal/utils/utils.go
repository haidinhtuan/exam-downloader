...
func CleanText(raw string) string {
	...
	// Remove "Suggested Answer: [Choices] 🗳️" pattern
	// Example: "Suggested Answer: ACE 🗳️"
	suggestedAnswerRegex := regexp.MustCompile(`Suggested Answer:\s*[A-Z, ]+\s*🗳️?`)
	cleaned = suggestedAnswerRegex.ReplaceAllString(cleaned, "")

	// Remove "Click the Exhibit button" and variations
	// Also remove "Referring to the exhibit" which often follows
	exhibitRegex := regexp.MustCompile(`(?i)(click the exhibit button\.?|click the exhibit\.?|referring to the exhibit,?|-- exhibit --)`)
	cleaned = exhibitRegex.ReplaceAllString(cleaned, "")

	// Removed the line that added newline before "Suggested Answer"
